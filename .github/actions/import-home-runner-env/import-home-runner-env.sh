...
while IFS= read -r entry; do
  case "$entry" in
    HTTP_PROXY=*|HTTPS_PROXY=*|ALL_PROXY=*|NO_PROXY=*|http_proxy=*|https_proxy=*|all_proxy=*|no_proxy=*)
      ...
      printf '%s\n' "$entry" >> "$github_env"
      printf '%s=set\n' "$variable_name"
      proxy_variable_count=$((proxy_variable_count + 1))
      ;;
  esac
done <<< "$shell_environment"

printf 'KINDOW_HOME_PROXY_VARIABLE_COUNT=%s\n' "$proxy_variable_count" >> "$github_env"
printf 'Imported %s proxy environment variable(s) from %s\n' "$proxy_variable_count" "$login_shell"
